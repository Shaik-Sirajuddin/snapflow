...
func New(h Handler) *server.MCPServer {
	...
	s.AddTools(
		...
		tool("daemon.list",
			"List daemon-owned and externally launched Snapflow instances. Use active=true to find live SAP-editable instances or active=false for inactive/stale rows.",
			mcp.WithBoolean("active", mcp.Description("Optional live-state filter; omitted returns both active and inactive instances.")),
			h,
			dynamicOutputSchema[ProcessInstanceList]()),
		tool("daemon.health",
...
var supportedSAPMethods = []sapMethodMetadata{
	...
	{Method: "edit.setTrackProperties", Description: "Partially update a track's mute/hidden/locked/blendMode/composite; omitted fields are left unchanged.", Params: `{trackIndex, muted?, hidden?, locked?, blendMode?, composite?}`},
	...
	{Method: "playlist.appendImageSequence", Description: "Append a numbered image sequence to the playlist.", Params: `{path, ttl?, begin?}`},
	...
	{Method: "playback.play", Description: "Start timeline playback.", Params: `{speed?}`},
	{Method: "playback.pause", Description: "Pause timeline playback.", Params: `{position?}`},
	{Method: "playback.stop", Description: "Stop timeline playback.", Params: "{}"},
	{Method: "playback.getState", Description: "Read transport playing/position/duration.", Params: "{}"},
	{Method: "playback.getFrame", Description: "Read a rendered frame from the bound project.", Params: `{frame, format?}`},
	{Method: "project.setProfile", Description: "Set canvas profile name or width/height/fps.", Params: `{profileName?} | {width, height, frameRateNum?, frameRateDen?}`},
	{Method: "project.getProfile", Description: "Read live canvas width/height/fps.", Params: "{}"},
