...
impl<'a, T: ProcessScene> SceneBuilder<'a, T> {
    ...
    fn draw_image_impl(
        ...
        // Vendored workaround (not upstream Slint), broader than the two
        // per-branch guards below: `physical_clip`'s cast a few lines
        // down (float -> pixel-coordinate int, via `.round().cast()`)
        // runs unconditionally, for *every* image, built from this
        // renderer's *current* clip/offset/scale state -- not anything
        // specific to the image being drawn. If any of those become
        // non-finite (inherited from a parent layout/Flickable's own
        // degenerate computation, e.g. a viewport size momentarily 0
        // feeding a division elsewhere in the item tree), that cast
        // panics via the same `euclid::Size2D`/`Point2D` `try_cast()` ->
        // `None` -> `unwrap()` path documented on the per-branch guards
        // below, before this function ever reaches either of them. Bail
        // out for this one frame rather than let it happen -- see those
        // guards' own comment for why this can't be caught any other way
        // in a `staticlib` (panic=abort).
        // `fit_size`/`image_fit_offset` (this function's own `FitResult`
        // parameter, computed by Slint core's layout/fit-mode logic
        // *before* this function is ever called) feed `target_rect`'s
        // cast a bit further down (`euclid::Rect::new(offset,
        // fit_size).round().cast()`) just as unconditionally as the
        // renderer-state fields above -- confirmed as a second, distinct
        // panic site (`euclid::Rect::cast::<i32>`, not
        // `Size2D::cast::<i32>`) via a second real reproduction. Guarded
        // here for the same reason.
        if !self.current_state.offset.x.is_finite()
            || !self.current_state.offset.y.is_finite()
            || !self.current_state.clip.origin.x.is_finite()
            || !self.current_state.clip.origin.y.is_finite()
            || !self.current_state.clip.size.width.is_finite()
            || !self.current_state.clip.size.height.is_finite()
            || !self.scale_factor.get().is_finite()
            || !fit_size.width.is_finite()
            || !fit_size.height.is_finite()
            || !image_fit_offset.x.is_finite()
            || !image_fit_offset.y.is_finite()
            || !source_to_target_x.is_finite()
            || !source_to_target_y.is_finite()
        {
            // TEMP DIAGNOSTIC (not upstream, remove after root-causing the
            // all-icons-blank regression): print exactly once which field
            // tripped this guard, since the guard's own early-return gives
            // no other signal that it fired at all.
            ...
            if !PRINTED.swap(true, std::sync::atomic::Ordering::Relaxed) {
                std::eprintln!(
                    "panel-rust DIAG draw_image_impl guard tripped: offset=({}, {}) clip.origin=({}, {}) clip.size=({}, {}) scale_factor={} fit_size=({}, {}) image_fit_offset=({}, {}) source_to_target=({}, {})",
                    self.current_state.offset.x, self.current_state.offset.y,
                    self.current_state.clip.origin.x, self.current_state.clip.origin.y,
                    self.current_state.clip.size.width, self.current_state.clip.size.height,
                    self.scale_factor.get(),
                    fit_size.width, fit_size.height,
                    image_fit_offset.x, image_fit_offset.y,
                    source_to_target_x, source_to_target_y,
                );
            }
            return;
        }
        ...
        match image_inner {
            ...
            ImageInner::StaticTextures(StaticTextures {
                ...
                // Vendored workaround (not upstream Slint) -- same
                // zero-size-division hazard as the SVG (`_`) branch's
                // `orig.width`/`orig.height` guard below; see that one's
                // comment for the full rationale.
                if original_size.width == 0 || original_size.height == 0 {
                    return;
                }
                // (original_size is an integer type here, so no separate
                // is_finite() check is needed the way the SVG branch's
                // float `orig` needs one -- an integer can't be NaN.)
            ...
            _ => {
                ...
                // Vendored workaround (not upstream Slint): `orig.width`/
                // `orig.height` feed a division a few lines down
                // (`buf_size.width / orig.width`) with no zero-guard --
                // if this image's reported intrinsic size is
                // momentarily 0 (an interim/not-yet-ready loading
                // state), that division produces `Infinity`, and the
                // `euclid::Size2D` cast a few lines later on an
                // `Infinity`/`NaN` value panics via `try_cast()`
                // returning `None` -- fatal for any host embedding this
                // renderer as a `staticlib` (panic=abort is implicit for
                // that crate-type, so this cannot be caught at the FFI
                // boundary either). Skipping this image for the current
                // frame is harmless: the next repaint (already scheduled
                // for anything still loading) draws it once a real size
                // is available.
                // `<= 0.0` alone is not enough: IEEE-754 says any
                // comparison against NaN is false, so a NaN width/height
                // (not just a zero one) would silently pass a bare
                // `<= 0.0` check and still reach the division below.
                // `is_finite()` correctly rejects NaN and +-Infinity too.
                if !orig.width.is_finite() || !orig.height.is_finite() || orig.width <= 0.0 || orig.height <= 0.0 {
                    static PRINTED: std::sync::atomic::AtomicBool = std::sync::atomic::AtomicBool::new(false);
                    if !PRINTED.swap(true, std::sync::atomic::Ordering::Relaxed) {
                        std::eprintln!("panel-rust DIAG SVG-branch guard tripped: orig=({}, {})", orig.width, orig.height);
                    }
                    return;
                }
