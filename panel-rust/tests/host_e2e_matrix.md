...
| Agent terminal | Prompt plus live output | `terminal/create`, output deltas, exit state, and release are recorded | Backend half proven (real acpx-server process, not this GUI harness): `acpx-server/tests/agent_request_fs_terminal_relay_test.rs`'s `ws_client_approves_terminal_create_and_receives_live_output_stream` (create + `acpx/terminal_created` + live output + exit) and `ws_client_initiated_kill_terminates_a_long_running_terminal` (client-initiated `terminal/kill` actually terminates a real process). GUI-level chip/popup/full-view rendering still Pending -- blocked on a compiled snapflow/shotcut binary for this worktree (see `memory/rui/gen/plans/background-terminals-ui/meta.json#background_terminals_runtime_gate`) |
